...
Cargo Workspace is a very powerfull tools, when building multiple binaries and reusable library. However if a cargo workspace is present, `now-rust` might fail to build.
For now, the solution is to add all your cargo workspace in `.gitignore` or `.nowignore`.
Given the project:

```
.
├── .nowignore <- will need to have `/Cargo.toml` and `/Cargo.lock` to be safe
├── Cargo.lock <- cargo workspace
